...
def cca_torch(X, Y, outdim_size, epsilon=1e-10):
    ...
    Wx = SigmaXX_inv_sqrt @ U[:, :outdim_size]  
    Wy = SigmaYY_inv_sqrt @ Vh.T[:, :outdim_size]

    return Wx, Wy, S[:outdim_size] 
...
if __name__ == "__main__":
    ...
    text_norm_cop = torch.nn.functional.normalize(text_cop, p=2, dim=1)
    ...
    t2i_ori_cca = t2i_sim_cca.sum() / t2i_sim_cca.shape[0]   
    ...
    i2t_ori_cca = i2t_sim_cca.sum() / i2t_sim_cca.shape[0]   
