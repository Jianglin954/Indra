...
def main():
    ...
    parser.add_argument("--ancors", type=int, default=1000)
    ...
    if args.dataset == "SVHN":
        ...
        train_dataset = DatasetClass(root="./data", split='train', download=True, transform=transform)
        test_dataset = DatasetClass(root="./data", split='test', download=True, transform=transform)
    else:
    ...
    if args.test_our_features:
        ...
        N, D = img_norm.shape
        anchor_size = args.ancors 
        anchor_indices = torch.randperm(N)[:anchor_size]
        anchor_imgs = img_norm[anchor_indices]


        if args.ori_dis == "Euclidean":
            print("Using Euclidean distance...")
            img_norm_cos_sim = torch.cdist(img_norm, anchor_imgs, p=2)  

        elif args.ori_dis == "Cosine":
            print("Using Cosine similarity...")
            img_norm_cos_sim = img_norm @ anchor_imgs.T  
